...
class enko_Oxford {
    ...
    async displayName() {
        ...
        if (locale.indexOf('KR') != -1) return 'Oxford EN->KO Dictionary';
        return 'Oxford EN->KO Dictionary';
    }
    ...
    async findOxford(word) {
        ...
        function buildDefinitionBlock(exp, pos, defs) {
            ...
            for (const def of defs) {
                ...
                if (def.tag == 'd' || def.tag == 'ud')
                    definition += pos + `<span class='tran'><span class='eng_tran'>${def.enText}</span><span class='kor_tran'>${def.koText}</span></span>`;
                if (def.tag == 'x' && sentnum < maxexample) {
                    sentnum += 1;
                    let enText = def.enText.replace(RegExp(exp, 'gi'), `<b>${exp}</b>`);
                    sentence += `<li class='sent'><span class='eng_sent'>${enText}</span><span class='kor_sent'>${def.koText}</span></li>`;
                }
            }
        ...
        let base = 'https://fanyi.baidu.com/v2transapi?from=en&to=ko&simple_means_flag=3';
            ...
            try {
                ...
                for (const def of symbols.parts)
                    if (def.means && def.means.length > 0) {
                        ...
                        definition += `<li class="ec">${pos}<span class="ec_kor">${def.means.join()}</span></li>`;
                    }
                definition += '</ul>';
                let css = `<style>
                ...
                for (const entry of entries) {
                    if (entry.tag == 'p-g' || entry.tag == 'h-g') {
                        ...
                        for (const group of entry.data) {
                            ...
                            if (group.tag == 'd') {
                                definition += pos + `<span class='tran'><span class='eng_tran'>${group.enText}</span><span class='kor_tran'>${group.koText}</span></span>`;
                            ...
                            if (group.tag == 'sd-g' || group.tag == 'ids-g' || group.tag == 'pvs-g') {
                                for (const item of group.data) {
                                    if (item.tag == 'sd') definition = `<div class="dis"><span class="eng_dis">${item.enText}</span><span class="kor_dis">${item.koText}</span></div>` + definition;
                                    let defs = [];
